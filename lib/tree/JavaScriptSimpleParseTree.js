...
class JavaScriptSimpleParseTree {
  ...
  toSourceCode() {
    ...
    while (queue.length > 0) {
      ...
      if (currentNode instanceof JavaScriptSimpleParseTreeNode) {
        ...
        if (lastTrailingText === null)
          lastTrailingText = trailingText;
    ...
    treeStack.unshift(lastTrailingText);
    return treeStack.reverse().join('');
  }
  ...
  generateFeatures() {
    ...
    const traverseTree = (_treeNode, _features, _leafTokens, _tokenUsageMap, _parents) => {
      if (_treeNode instanceof JavaScriptSimpleParseTreeNode && _treeNode.isLeaf === true) {
        let tokenName;
        if (_treeNode.isVariable === true && !_treeNode.isCapitalized)
          tokenName = VAR_NAME;
        else
  ...
  static computeSimilarityScore(tree1, tree2) {
    const tree1Features = tree1.features ? tree1.features : tree1.generateFeatures();
    const tree2Features = tree2.features ? tree2.features : tree2.generateFeatures();
    const differences = tree1Features.filter(function(v) {
      return !this.get(v) || !this.set(v, this.get(v) - 1);
    }, tree2Features.reduce( (acc, v) => acc.set(v, (acc.get(v) || 0) + 1), new Map() ));

    const similarityScore = (tree1Features.length - differences.length) * 100 / tree1Features.length;
...
export default JavaScriptSimpleParseTree;